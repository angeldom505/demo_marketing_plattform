-- Cache de analytics: lectura instantánea desde API, refresh en background.
-- La tabla guarda el resultado pre-calculado en JSONB.
-- get_hubspot_analytics()       → lee de cache (< 5ms)
-- refresh_hubspot_analytics()   → recalcula y guarda (llamado desde sync o CLI)
...
-- Función de refresh: hace el trabajo pesado, guarda en cache.
-- Se llama desde supabase CLI o desde el script de sync (conexión directa, sin timeout PostgREST).
...
-- Función de lectura: instantánea, lee solo del cache
...
STABLE
AS $$
  SELECT data FROM hubspot_analytics_cache WHERE id = 1;
$$;

-- RLS en cache
ALTER TABLE hubspot_analytics_cache ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "service_role_all_cache" ON hubspot_analytics_cache;
CREATE POLICY "service_role_all_cache" ON hubspot_analytics_cache
  FOR ALL TO service_role USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "authenticated_read_cache" ON hubspot_analytics_cache;
CREATE POLICY "authenticated_read_cache" ON hubspot_analytics_cache
  FOR SELECT TO authenticated USING (true);

-- Permisos
REVOKE ALL ON FUNCTION get_hubspot_analytics()       FROM PUBLIC;
REVOKE ALL ON FUNCTION refresh_hubspot_analytics()   FROM PUBLIC;
GRANT EXECUTE ON FUNCTION get_hubspot_analytics()     TO service_role, authenticated;
GRANT EXECUTE ON FUNCTION refresh_hubspot_analytics() TO service_role;
