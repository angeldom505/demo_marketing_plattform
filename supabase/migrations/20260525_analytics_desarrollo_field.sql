-- Corregir campo de desarrollo: usar solo `desarrollo` (asignación CRM),
-- no `desarrollo_interes` (interés de compra del contacto).

-- ── 1. Refresh cache: by_desarrollo usa solo campo `desarrollo` ──────────
...
BEGIN
  ...
  des AS MATERIALIZED (
    SELECT COALESCE(NULLIF(desarrollo,''), 'Sin asignar') AS k, count(*) AS cnt
    FROM c GROUP BY k ORDER BY cnt DESC
  ),
  mod AS MATERIALIZED (
...
-- ── 2. Filtro por desarrollo: usa solo campo `desarrollo` ────────────────
...
BEGIN
  WITH c AS MATERIALIZED (
    SELECT
      ...
      city
    FROM hubspot_contactos
    WHERE COALESCE(NULLIF(desarrollo,''), 'Sin asignar') = p_desarrollo
  ),
  lc AS MATERIALIZED (
    SELECT COALESCE(NULLIF(lifecyclestage,''),'sin_etapa') AS k, count(*) AS cnt
    FROM c GROUP BY k ORDER BY cnt DESC
...
-- ── 3. Periodo: también usa solo `desarrollo` ────────────────────────────
